...
public class PrintImage {
    ...
    public static  void writeAvatar(String name,boolean thumb, HttpServletRequest request, HttpServletResponse response) {
        String photoBase = Config.avatarPath;
        File file;
        if (!photoBase.endsWith("/")) {
            photoBase += '/';
        }
        ...
        photoBase = Config.downloadAvatarPath;
        if (!photoBase.endsWith("/")) {
            photoBase += '/';
        }
        file = new File(photoBase + name +".jpg");
        if (file.exists()) {
                //write photo to the stream
                write(file, response, request.getSession().getServletContext());
                return;
        ...
        write(new File(Config.defaultAvatar), response, request.getSession().getServletContext());
